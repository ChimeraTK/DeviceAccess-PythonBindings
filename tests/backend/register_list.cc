...
namespace TestBackend {
  ...
  class TBRegisterInfo : public ChimeraTK::RegisterInfo {
   ...
   public:
    TBRegisterInfo(ChimeraTK::RegisterPath name, size_t elements, size_t channels, size_t dimensions,
    ...
    unsigned int getNumberOfElements() const override { return static_cast<unsigned int>(elements_); }
    unsigned int getNumberOfChannels() const override { return static_cast<unsigned int>(channels_); }
    unsigned int getNumberOfDimensions() const override { return static_cast<unsigned int>(dimensions_); }
    DataDescriptor const& getDataDescriptor() const override { return descriptor_; }
  ...
  std::tuple<size_t, size_t, size_t> convertToChimeraTkShape(Register::Shape const& s);
  ChimeraTK::RegisterInfo::DataDescriptor getChimeraTkRegisterDescriptor(Register const& r);
  ...
  boost::shared_ptr<TBRegisterInfo> getChimeraTkRegisterInfo(Register const& r) {
    ...
    size_t elements;
    size_t dimensions;
    std::tie(channels, elements, dimensions) = convertToChimeraTkShape(r.getShape());

    return boost::shared_ptr<TBRegisterInfo>(new TBRegisterInfo(r.getName(), //
        elements,                                                            //
        channels,                                                            //
        dimensions,                                                          //
        r.getAccessMode(),                                                   //
