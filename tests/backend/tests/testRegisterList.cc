...
BOOST_FIXTURE_TEST_CASE(testCatalogue, Fixture_t) {
  ...
  for(auto& it : catalogue) {
    if(it.getRegisterName() == "test1") {
      ...
      BOOST_CHECK(it.getDataDescriptor().fundamentalType() == ChimeraTK::RegisterInfo::FundamentalType::numeric);
    }
      ...
      BOOST_CHECK(it.getDataDescriptor().fundamentalType() == ChimeraTK::RegisterInfo::FundamentalType::string);
    }
    else {
