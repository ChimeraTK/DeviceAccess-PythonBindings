...
namespace mtca4upy {
  ...
  inline void throwExceptionIfOutOfBounds(const mtca4upy::NumpyObject& dataToWrite, const size_t& bytesToWrite) {
    size_t bytesInArray = (extractNumberOfElements(dataToWrite) * sizeof(int32_t));
    if(bytesInArray < bytesToWrite) {
      throw mtca4upy::ArrayOutOfBoundException("size to write is more than the supplied array size");
    }
  }

  inline mtca4upy::numpyDataTypes extractDataType(const mtca4upy::NumpyObject& Buffer) {
    PyArrayObject* numpyArrayObj = reinterpret_cast<PyArrayObject*>(Buffer.ptr());
